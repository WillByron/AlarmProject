...
Set @max = 0
		...
		SELECT [AuditId]
				  ...
				  ,[channel]
				  ,[Duration]
				  ,lag(Duration)OVER (partition BY a.Rank  Order by timestamp) PreviousDuration
				  ,lead(Duration)OVER (partition BY a.Rank  Order by timestamp) NextDuration
				  ,[text]
				  ,[msh_ts]
				  ,[alarm_ts]
				  ,[msg_id]
		...
		Begin
			...
			SELECT [AuditId]
				  ...
				  ,[channel]
				  ,[Duration]
				  ,lag(Duration)OVER (partition BY UnionAll.Rank  Order by timestamp) PreviousDuration
				  ,lead(Duration)OVER (partition BY UnionAll.Rank  Order by timestamp) NextDuration
				  ,[text]
				  ,[msh_ts]
				  ,[alarm_ts]
				  ,[msg_id]
			...
			from (	
				...
				(select AuditId,a.[Rank],[source],[unit],[bed],[channel],Duration,[text],[msh_ts],[alarm_ts],[msg_id],[TimeStamp],[filename],[DateInserted],[AlarmEnded],0
				 FROM HelixReportStage.[dbo].HadoopRawAlarmsStage a 
				left outer join (select distinct rank FROM HelixReportStage.[dbo].HadoopRawAlarmsStage where filename   in  (select max(filename) FROM HelixReportStage.[dbo].HadoopRawAlarmsStage))b
				on a.rank = b.rank where b.rank is not null ) ) UnionAll

				CREATE Unique CLUSTERED INDEX IDX_ID ON #MyTable2(Loopid)
				SELECT @counter = Min(Loopid) ,@max = Max(Loopid),@MaxFileName =max(filename) FROM #MyTable2
		End

		--select * from #MYTable2 where source = 'HSRIXSERV5' and unit = 'V3S' and bed = 'V3S3007' and channel = '286' and text = '!! ECG Leads Off'
		--select * from #MYTable2 order by loopid
					...
					BEGIN
							...
							Begin
    							SELECT 
									...
									@msh_ts=msh_ts ,@alarm_ts=alarm_ts ,@msg_id=msg_id ,@TIMESTAMP=TIMESTAMP ,@filename=filename ,@DateInserted=DateInserted ,@AlarmEnded=AlarmEnded,
