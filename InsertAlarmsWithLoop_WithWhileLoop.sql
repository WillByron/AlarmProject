...
SELECT @counter = Min(LoopId) ,@max = Max(LoopId),@LastFilename = max(filename) FROM HelixReportStage.[dbo].HadoopRawAlarmsStage 
...
WHEN MATCHED THEN
	...
	case	when Source.Duration > 15 then isnull(Target.Duration,0) + 5 when source.Duration = 0 and source.filename <> @LastFilename  then isnull(Target.Duration,0)  
